...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> _loadFavoriteChannels() async {
    ...
    setState(() {
      favoriteChannels =
          channelJsonList.map((json) => ChannelInfo.fromJson(json)).toList();
      filteredChannels = favoriteChannels;
      groupedChannels = _groupChannelsByCategory(filteredChannels);
    });
  }

  Map<String, List<ChannelInfo>> _groupChannelsByCategory(
    ...
    if (groupedChannels.isNotEmpty) {
      return DefaultTabController(
        ...
        child: Scaffold(
            ...
            actions: [
              IconButton(
                onPressed: () {
                  showSearch(
                      context: context,
                      delegate: ChannelSearchDelegate(filteredChannels));
                },
          ...
          body: Container(
            // Wrap the body content in a Container
            color: Colors.cyan.shade50,
            child: TabBarView(
              children: groupedChannels.keys.map((category) {
                List<ChannelInfo> channelsInCategory =
